...
module.exports = app => {
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      try {
        ...
        const room = await Room.findOne({ building: building.id, roomNumber })
          .populate({
            path: 'classes',
            select: { room: false, building: false },
            populate: {
  ...
  app.post('/api/search/buildings', async (req, res) => {
    ...
    try {
      const selectedBuildings = await Building.find({
        buildingCode: {
          $in: buildings
        }
      })
      ...
      const occupiedClassSlots = await ClassSlot.find({
        day,
        $and: [
          {
            $or: [
      ...
      const emptyRoomsResult = selectedBuildings.map(building => {
        ...
        modifiedBuilding.rooms = building.rooms.filter(
          room =>
            !occupiedClassSlots.some(classSlot =>
              classSlot.room.equals(room._id)
            )
        );
